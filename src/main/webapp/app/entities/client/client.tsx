          ...
          <Table responsive>
            <thead>
              <tr>
                ...
                <th className="hand" onClick={sort('advocate')}>
                  Advocate <FontAwesomeIcon icon="sort" />
                </th>
                ...
                <th className="hand" onClick={sort('oabnumber')}>
                  Oabnumber <FontAwesomeIcon icon="sort" />
                </th>
            ...
            <tbody>
              {clientList.map((client, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{client.advocate ? 'true' : 'false'}</td>
                  <td>{client.biography}</td>
                  <td>{client.birthdate ? <TextFormat type="date" value={client.birthdate} format={APP_LOCAL_DATE_FORMAT} /> : null}</td>
                  <td>{client.firstname}</td>
                  <td>{client.lastname}</td>
                  <td>{client.oabnumber}</td>
                  <td>{client.phone}</td>
                  <td>{client.createdAt ? <TextFormat type="date" value={client.createdAt} format={APP_LOCAL_DATE_FORMAT} /> : null}</td>
                  <td>
                    {client.picture ? (
                      <div>
                        {client.pictureContentType ? (
                          <a onClick={openFile(client.pictureContentType, client.picture)}>
                            <img src={`data:${client.pictureContentType};base64,${client.picture}`} style={{ maxHeight: '30px' }} />
                            &nbsp;
                          </a>
                        ) : null}
                        <span>
                          {client.pictureContentType}, {byteSize(client.picture)}
                        </span>
                      </div>
                    ) : null}
                  </td>
                  <td>{client.credential ? <Link to={`credential/${client.credential.id}`}>{client.credential.id}</Link> : ''}</td>
