...
export const Chat = (props: IChatProps) => {
  ...
  const { chatList, match, loading, totalItems } = props;
  return (
    <div>
      ...
      <div className="table-responsive">
        {chatList && chatList.length > 0 ? (
            ...
            <tbody>
              {chatList.map((chat, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{chat.createdAt ? <TextFormat type="date" value={chat.createdAt} format={APP_LOCAL_DATE_FORMAT} /> : null}</td>
          ...
          !loading && <div className="alert alert-warning">No Chats found</div>
        )}
      </div>
      {props.totalItems ? (
